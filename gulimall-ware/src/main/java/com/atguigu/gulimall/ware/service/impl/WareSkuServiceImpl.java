...
//           todo 远程查询sku名字，如果失败无需回滚

//            自己catch异常
//            todo 还可以用什么方法让异常出现不会滚
            try {
    ...
    public boolean orderLockStock(WareSkuLockVo vo) {
        ...
        for (SkuWareHasStock hasStock : collect) {
            ...
            for (Long aLong : wareId) {
                Long aLong1 = wareSkuDao.lockSkuStock(skuId, aLong, hasStock.getNum());
                if (aLong1 == 1) {
                    skuStocked = true;
//                    todo 告诉mq锁定成功
                    WareOrderTaskDetailEntity wareOrderTaskDetailEntity = new WareOrderTaskDetailEntity(null,skuId,"",hasStock.getNum(),wareOrderTaskEntity.getId(),aLong,1);
